...
def transcribe_whisper(args, model, loader, split):
    ...
    for audio_or_mels, texts, audio_path, accent, domain, vad in tqdm(loader):
        if "whisper" in args.model_id_or_path and os.path.isdir(args.model_id_or_path):
            ...
            predicted_class_ids = torch.argmax(torch.tensor(logits), dim=-1)
            predicted_labels = [model.config.id2label[label.item()] for label in predicted_class_ids]
            accent_int = [model.config.label2id.get(acnt, model.config.label2id['unk']) for acnt in accent]

        references.extend(texts)
        hypotheses.extend(predicted_labels)
        hypotheses_int.extend(predicted_class_ids.tolist())
        paths.extend(audio_path)
        accents.extend(accent)
        accents_ints.extend(accent_int)
    ...
    write_pred_inference_df(args.model_id_or_path, data, f1['f1'], split=split)
...
if __name__ == "__main__":
    ...
    if "whisper" in args.model_id_or_path and os.path.isdir(args.model_id_or_path):
        ...
        processor = AutoFeatureExtractor.from_pretrained(args.model_id_or_path)
        model = AutoModelForAudioClassification.from_pretrained(args.model_id_or_path).to(device)
